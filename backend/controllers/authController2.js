...
exports.signup = (req, res) => {
  ...
  db.query(checkQuery, [email, username], (err, result) => {
    ...
    const insertQuery =
      'INSERT INTO users (fname, lname, username, phone, email, pw) VALUES (?, ?, ?, ?, ?, ?)';
...
exports.login = (req, res) => {
  ...
  db.query(query, [email, pw], (err, results) => {
    ...
    if (results.length > 0) {
      return res.status(200).json({ success: true, redirect: '/home.html' }); // Change this to your actual homepage
    } else {
